...
function createRecordId(type, record, index) {
  const parts = [type, record.state || "", record.city || "", record.name || record.medicine || record.company || `item-${index}`]
    .filter(Boolean)
    .join("-");
...
function addIndiaWideCoverage() {
  ...
  indiaCoverage.forEach((region, index) => {
...
function buildLocationLine(item) {
  return [item.locality, item.city, item.state].filter(Boolean).join(", ");
}
...
async function syncBackendData() {
  try {
    ...
    if (needsSeed) {
      await apiRequest("/seed", { method: "POST", body: seedData });
...
function getStatesFromAllData() {
  return getUnique([...donors, ...bloodBanks, ...hospitals, ...medicineInventory, ...diagnostics, ...pharmacies], "state");
}

function getCitiesForState(state) {
  const sources = [...donors, ...bloodBanks, ...hospitals, ...diagnostics, ...pharmacies];
  const scoped = state === "all" ? sources : sources.filter((item) => item.state === state);
  return getUnique(scoped, "city");
}

function fillCitySelect(select, state, placeholder) {
  fillSelect(select, getCitiesForState(state), placeholder);
}

function matchesLocation(item, state, city, locality = "") {
  ...
  return matchesState && matchesCity && matchesLocality;
}
...
function renderQuickFinderStats() {
  ...
  elements.quickHospitalCount.textContent = hospitals.filter((item) => matchesLocation(item, state, city, locality)).length;
  elements.quickBloodbankCount.textContent = bloodBanks.filter((item) => matchesLocation(item, state, city, locality)).length;
  elements.quickLabCount.textContent = diagnostics.filter((item) => matchesLocation(item, state, city, locality)).length;
  elements.quickPharmacyCount.textContent = pharmacies.filter((item) => matchesLocation(item, state, city, locality)).length;
}
...
function syncDirectoryCityOptions() {
  ...
  const filteredBanks = selectedState === "all"
    ? bloodBanks
    : bloodBanks.filter((bank) => bank.state === selectedState);
  const filteredDonors = selectedState === "all"
    ? donors
    : donors.filter((donor) => donor.state === selectedState);
  const cityOptions = getUnique([...filteredBanks, ...filteredDonors], "city");

  fillSelect(elements.directoryCity, cityOptions, "All cities");
}
...
function renderDirectory() {
  ...
  const donorResults = donors.filter((donor) => {
    const matchesQuery = [donor.name, donor.city, donor.state].some((field) =>
    ...
    return matchesQuery && matchesState && matchesCity && matchesBlood;
  });

  const bankResults = bloodBanks.filter((bank) => {
    const matchesQuery = [bank.name, bank.city, bank.state].some((field) =>
      field.toLowerCase().includes(query)
    );
        ...
        .map(
          (donor) => `
            <article class="directory-card">
              ...
              <p class="card-meta">${donor.city}, ${donor.state}</p>
              <div class="card-badges">
        ...
        .map((bank) => {
          ...
          return `
            <article class="bloodbank-card">
              ...
              <p class="card-meta">${bank.city}, ${bank.state}</p>
              <div class="card-badges">
...
function renderHospitals() {
  ...
  const results = hospitals.filter((hospital) => {
    const matchesQuery = [hospital.name, hospital.city, hospital.state].some((field) =>
      field.toLowerCase().includes(query)
    );
    ...
    return matchesQuery && matchesState && matchesService && matchesHighway;
  });
...
function renderMedicine() {
  ...
  const results = medicineInventory.filter((item) => {
    const matchesQuery = [item.medicine, item.company, item.city].some((field) =>
      field.toLowerCase().includes(query)
    );
    const matchesState = state === "all" || item.state === state;
...
function renderDiagnostics() {
  ...
  const results = diagnostics.filter((item) => {
    const matchesQuery = [item.name, item.locality, item.city].some((field) =>
      field.toLowerCase().includes(query)
    );
    const matchesState = state === "all" || item.state === state;
...
function renderEmergencyResources() {
  ...
  const rankedHospitals = [...hospitals]
    .filter((hospital) => matchesLocation(hospital, state, city, locality))
  ...
  const rankedBloodbanks = [...bloodBanks]
    .filter((bank) => matchesLocation(bank, state, city, locality))
    .map((bank) => ({ ...bank, score: bank.city === city ? 2 : 0 }))
    .sort((a, b) => b.score - a.score)
    .slice(0, 3);

  const rankedPharmacies = [...pharmacies]
    .filter((pharmacy) => matchesLocation(pharmacy, state, city, locality))
    ...
    .map(
      (hospital) => `
        <article class="emergency-card">
          ...
          <p class="card-meta">${hospital.locality}, ${hospital.city}</p>
          <div class="card-badges">
    ...
    .map(
      (pharmacy) => `
        <article class="emergency-card">
          ...
          <p class="card-meta">${pharmacy.locality}, ${pharmacy.city}</p>
          <div class="card-badges">
...
function syncQuickCityOptions() {
  fillCitySelect(elements.quickCity, elements.quickState.value, "All cities");
}

function syncEmergencyCityOptions() {
  fillCitySelect(elements.emergencyCity, elements.emergencyState.value, "Select city");

  if (elements.emergencyCity.value === "all") {
    const [firstCity] = getCitiesForState(elements.emergencyState.value);
...
function registerForms() {
  elements.donorForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      addActivity("Donor", savedDonor.name, `${savedDonor.city}, ${savedDonor.state} donor registration added to verification queue.`);
      ...
      setFormMessage(
        elements.donorForm,
        backendAvailable
          ? `Donor registered successfully and stored in the ${databaseMode} backend.`
          : "Donor registered in the current browser session. Start the backend server to save permanently."
      );
    } catch (error) {
  ...
  elements.hospitalForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      setFormMessage(
        ...
        backendAvailable
          ? `Hospital partner registered in the ${databaseMode} backend.`
          : "Hospital partner added to the current browser session only."
      );
    } catch (error) {
  ...
  elements.pharmacyForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      setFormMessage(
        ...
        backendAvailable
          ? `Pharmacy registered in the ${databaseMode} backend.`
          : "Pharmacy added to the current browser session only."
      );
    } catch (error) {
    ...
    try {
      ...
      setFormMessage(
        ...
        backendAvailable
          ...
          : `Thank you ${name}. Your request is visible in this browser, but backend email delivery is not active yet.`
      );
    } catch (error) {
...
async function init() {
  ...
  addActivity(
    ...
    backendAvailable ? "Backend connected" : "Browser session"
  );
